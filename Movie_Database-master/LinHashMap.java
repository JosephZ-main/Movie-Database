       ...
       implements Serializable, Cloneable, Map <K, V>
{
    ...
    public TreeSet <Map.Entry <K, V>> entrySet ()
    {
        ...
        // Goes through hashtable to all the pairs
        for (int i = 0; i < hTable.size(); i++) {
            Bucket bucket = hTable.get(i);

            // Goes through the bucket chain at a specific position within the hashtable
            while (bucket != null) {
                // Iterates through the pairs stored within the bucket and adds them to the set
    ...
    private void split ()
    {
        ...
        for (int j = 0; j < oldBucket.nKeys; j++) {
            ...
            int h = h (k);
            if (h != i) {
                hTable.get(h % hTable.size()).add(k, oldBucket.value[j]);

            } else {
                newBucket.add (k, oldBucket.value[j]);
